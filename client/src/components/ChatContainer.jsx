...
const ChatContainer = () => {
    ...
    const { messages, selectedUser, setSelectedUser, sendMessage,
        getMessages, isTyping, deleteMessage, editMessage, deleteConversation } = useContext(ChatContext)
    ...
    return selectedUser ? (
        <div className='h-full overflow-hidden relative flex flex-col bg-transparent'>
            ...
            <div className='flex items-center gap-4 py-4 px-6 border-b border-white/10 bg-black/5 backdrop-blur-md shadow-sm z-10'>
                ...
                <button onClick={() => { if (confirm("Are you sure you want to delete the entire conversation?")) deleteConversation(selectedUser._id); }} className='p-2 hover:bg-white/10 rounded-full text-red-500/70 hover:text-red-500 transition-all' title="Delete Conversation">
